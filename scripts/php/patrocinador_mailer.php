...
$name = trim($_POST['nome_patrocinador']);
//pega os dados que foi digitado no ID name.

$email = trim($_POST['email_patrocinador']);
//pega os dados que foi digitado no ID email.

$empresa = trim($_POST['empresa_patrocinador']);

$whatsapp = trim($_POST['whatsapp_patrocinador']);
...
function validar_email($nome, $whatsapp, $message,$corpo, $headers, $email_to){
    if($nome === "" or $whatsapp == "" or $message === ""){
        gravar($corpo."data/hora: ".$agora = date('d/m/Y H:i').";\n");
        $status = false;
    }else{
        gravar($corpo."data/hora: ".$agora = date('d/m/Y H:i').";\n");
        $status = mail($email_to, $whatsapp, $corpo, $headers);
        //enviando o email.
    }
    return $status;
}
